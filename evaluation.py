...
def model_eval_multitask(
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(
            ...
            logits, loss = model.predict_similarity(
        ...
        for step, batch in enumerate(
            ...
            logits, loss = model.predict_sentiment(b_ids, b_mask, b_labels)
            sst_loss += loss.item()
        ...
        print(f"Paraphrase detection loss: {para_loss:.3f}")
        print(f"Sentiment classification loss: {sst_loss:.3f}")
        print(f"Semantic Textual Similarity loss: {sts_loss:.3f}")
...
#         # Evaluate paraphrase detection.
#         for step, batch in enumerate(
#             tqdm(paraphrase_dataloader, desc=f"predict test para")
#         ):
#             (b_ids1, b_type, b_mask1, b_sent_ids) = (
#                 batch["token_ids"],
#                 batch["token_type_ids"],
#                 batch["attention_mask"],
#                 batch["sent_ids"],
...
#             logits = model.predict_paraphrase(b_ids1, b_type, b_mask1)
#             y_hat = logits.sigmoid().round().flatten().cpu().numpy()
...
#         # Evaluate semantic textual similarity.
#         for step, batch in enumerate(
#             tqdm(sts_dataloader, desc=f"predict test sts", disable=TQDM_DISABLE)
#         ):
#             (b_ids1, b_type, b_mask1, b_sent_ids) = (
#                 batch["token_ids"],
#                 batch["token_type_ids"],
...
#             logits = model.predict_similarity(b_ids1, b_type, b_mask1)
#             y_hat = logits.flatten().cpu().numpy()
...
#         # Evaluate sentiment classification.
#         for step, batch in enumerate(
#             tqdm(sentiment_dataloader, desc=f"predict test sst", disable=TQDM_DISABLE)
#         ):
...
def eval_test_model_multitask(args, model, device):
    ...
    # I don't need to submit prediction, can skip this part.
    # (
    #     test_para_y_pred,
    #     test_para_sent_ids,
    #     test_sst_y_pred,
